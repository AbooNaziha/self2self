...
def maxpool2d(x, k=2):
    ksize = [1, 1, k, k]
    return tf.nn.max_pool(x, ksize=ksize, strides=ksize, padding='SAME', data_format='NHWC')
...
def partial_conv_unet(x, mask, height=256, width=256, channel=3, p=0.7, **_kwargs):
...
def build_denoising_unet(noisy, p=0.7, is_realnoisy=False):
    ...
    response = partial_conv_unet(response, mask_tensor, channel=c, height=h, width=w, p=p)
